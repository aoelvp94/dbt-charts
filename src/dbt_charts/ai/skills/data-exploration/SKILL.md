...
Explore the schema with your own SQL through `{{ s_execute_query }}` before
...
5. **Sample only after narrowing candidates.** Use `{{ s_execute_query }}` on
...
When sampling with `{{ s_execute_query }}`:
