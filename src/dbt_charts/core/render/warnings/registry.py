...
place:
  ...
  GEOMETRY-dependent: reads ``ctx.vega_specs`` or another WarningContext field
    ...
    ``text_truncations``, ``static_pagination_caps``). A chart that never
  ...
  DATA-dependent: reads only ``ctx.chart_results`` (the query's actual rows)
    and/or resolved chart config off ``ctx.board_spec``. The query executed
    and the result is what it is regardless of whether anything got painted
    — these must run for every chart whose query ran, on-screen or not.
    ``chart_results`` is therefore built unconditionally in ``renderer.py``.
"""
