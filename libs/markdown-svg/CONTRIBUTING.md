...
This library lives inside the `dataface` monorepo as a peer package of `dbt-charts` — it has no `pyproject.toml` `[project]`/`[build-system]` of its own and cannot be installed standalone with `pip`. See `AGENTS.md` in this directory for the actual dev workflow (`just markdown_svg test`, etc.).

## Getting Started

### Setting Up Your Development Environment

1. **Clone the monorepo and bootstrap it**
   ```bash
   git clone https://github.com/fivetran/dataface.git
   cd dataface
   UV_PYTHON_PREFERENCE=only-managed just install
   ```

2. **Verify your setup**
   ```bash
   just markdown_svg test
   ```
