...
This directory has no `[project]`/`[build-system]` of its own and is not an independent uv workspace member — it is a peer package built directly into the `dbt-charts` wheel (`dataface/pyproject.toml`'s `[tool.hatch.build.targets.wheel.force-include]`), the same way `d3_format` is. Import name stays `mdsvg`.
