"""Tests for the chart.data_table primitive (authoring + validation).

These tests describe the v1 authoring surface behaviour — entry shape,
grammar rules G1..G5, validation rules from the chart.data_table spec §3.
"""
...
def test_data_table_source_entry_minimum():
    entry = ChartDataTableSource.model_validate({"source": "revenue"})
    assert entry.source == "revenue"
    assert entry.format is None
    assert entry.label is None


def test_data_table_source_entry_with_format_and_label():
    entry = ChartDataTableSource.model_validate(
...
def test_data_table_source_entry_requires_source():
    with pytest.raises(ValidationError):
        ChartDataTableSource.model_validate({"format": ",d"})


def test_data_table_source_entry_rejects_unknown_keys():
    # `extra=forbid` — no `column:`, `field:`, no inline styling per spec §2.3
    with pytest.raises(ValidationError):
        ChartDataTableSource.model_validate({"source": "revenue", "column": "x"})
    with pytest.raises(ValidationError):
        ChartDataTableSource.model_validate({"source": "revenue", "style": {}})
...
def test_data_table_aggregate_entry_minimum():
    entry = ChartDataTableAggregate.model_validate(
...
def test_data_table_aggregate_entry_requires_source():
    # Grammar rule G2 — aggregate always requires source.
    with pytest.raises(ValidationError) as excinfo:
        ChartDataTableAggregate.model_validate({"aggregate": "sum"})
...
def test_data_table_aggregate_accepts_supported_ops(op):
    entry = ChartDataTableAggregate.model_validate(
...
def test_data_table_aggregate_rejects_unknown_ops(op):
    # Grammar rule G4 — exact names, no aliases.
    with pytest.raises(ValidationError):
        ChartDataTableAggregate.model_validate({"aggregate": op, "source": "revenue"})
...
def test_chart_patch_data_table_mixed_entries():
    ...
    assert patch.data_table is not None
    assert len(patch.data_table.entries) == 2
    assert isinstance(patch.data_table.entries[0], ChartDataTableSource)
    assert isinstance(patch.data_table.entries[1], ChartDataTableAggregate)


def test_chart_patch_data_table_preserves_list_order():
    ...
    assert [e.source for e in patch.data_table.entries] == ["a", "b", "c"]


def test_chart_patch_data_table_disambiguates_source_vs_aggregate():
...
def test_chart_patch_empty_data_table_list_rejects():
...
def test_chart_patch_data_table_optional():
    # Default state: no data_table block present.
    patch = _chart_patch_adapter.validate_python(
        {"type": "bar", "x": "month", "y": "revenue"}
    )
    assert patch.data_table is None
...
def test_chart_patch_data_table_accepts_supported_chart_types(chart_type):
    patch = _chart_patch_adapter.validate_python(
        {
            "type": chart_type,
            "x": "month",
...
def test_chart_patch_data_table_rejects_unsupported_chart_types(chart_type, payload):
    # data_table is only declared on cartesian family patches (bar/line/area) and
    # layered. All other chart types forbid it via extra="forbid" on their patch
    # class, or via the _validate_data_table validator on _CartesianChartFields.
    full = {
        "type": chart_type,
        "data_table": [{"source": "y"}],
        **payload,
...
# data_table on a 5-tick chart is valid.


def test_chart_patch_data_table_allows_many_rows_at_authoring_time():
    ...
    assert len(patch.data_table.entries) == 60
...
def test_chart_patch_data_table_rejects_duplicate_source_only_entries():
    with pytest.raises(ValidationError) as excinfo:
        _chart_patch_adapter.validate_python(
            {
                "type": "bar",
...
def test_chart_patch_data_table_rejects_duplicate_aggregate_entries():
    with pytest.raises(ValidationError) as excinfo:
        _chart_patch_adapter.validate_python(
            {
                "type": "bar",
...
def test_chart_patch_data_table_allows_same_source_with_different_aggregate():
...
def test_chart_patch_data_table_allows_raw_and_aggregate_of_same_column():
...
def test_chart_patch_rejects_data_table_on_multi_y_list():
    # A chart with y: [a, b] cannot carry a data_table in v1 — the x→multiple-rows
...
def test_chart_data_table_supported_types_are_all_real_chart_types():
    ...
    for t in CHART_DATA_TABLE_SUPPORTED_TYPES:
        assert t in real_types, f"{t!r} is not a valid ChartType value"


# =============================================================================
# CONTAINER — ChartDataTable wrapper structure
# =============================================================================


def test_chart_data_table_container_accepts_plain_list():
...
def test_per_series_entry_minimum():
    ...
    entry = ChartDataTablePerSeries.model_validate({"per_series": "revenue"})
...
def test_per_series_entry_with_format():
    ...
    entry = ChartDataTablePerSeries.model_validate(
...
def test_per_series_entry_rejects_unknown_keys():
    ...
    with pytest.raises(ValidationError):
        ChartDataTablePerSeries.model_validate(
...
def test_per_series_entry_is_parsed_from_chart_data_table():
    from dbt_charts.core.compile.models.chart.authored import (
        ChartDataTablePerSeries,
    )
    ...
    assert isinstance(table.entries[0], ChartDataTablePerSeries)
...
def test_chart_patch_accepts_per_series_entry_with_color():
    ...
    assert patch.data_table is not None
    assert isinstance(patch.data_table.entries[0], ChartDataTablePerSeries)
...
def test_chart_patch_rejects_mixed_per_series_and_aggregate_in_same_list():
    ...
    with pytest.raises(ValidationError):
        ChartDataTable.model_validate([{"per_series": "revenue", "aggregate": "sum"}])


# =============================================================================
# by_measure field on ChartDataTablePerSeries
# =============================================================================
...
def test_data_table_entry_format_inherits_chart_format_at_normalize_time():
    """After normalize_chart(), a source entry reading chart.y inherits style.number_format."""
    from dbt_charts.core.compile.models.chart.authored import ChartDataTableSource
    ...
    entries = chart.data_table.entries
    assert isinstance(entries[0], ChartDataTableSource)
    assert entries[0].format == "~s", "measure entry must inherit style.number_format"
    assert isinstance(entries[1], ChartDataTableSource)
...
def test_resolved_chart_bakes_number_format_for_numeric_y():
    """ResolvedChart.data_table stamps the theme default number format onto
    ...
    apply_measure_format_to_data_table or re-derives the numeric gate
    itself."""
    ...
    entries = resolved.data_table.entries
    assert isinstance(entries[0], ChartDataTableSource)
    # "number_default" is predefined — the format name is stamped directly
    # (no theme formats dict lookup needed).
    assert entries[0].format == "number_default"
    assert isinstance(entries[1], ChartDataTableSource)
...
def test_resolved_chart_does_not_stamp_number_format_for_numeric_string_y():
    ...
    chart = BarChart(
        ...
        query=SqlQuery(sql="SELECT 1", source="test"),
        query_name="q",
        data_table=ChartDataTable.model_validate({"entries": [{"source": "revenue"}]}),
    )
    data = [{"month": "Jan", "revenue": "100"}, {"month": "Feb", "revenue": "200"}]
    resolved = resolve(chart, data, board_style)

    assert resolved.data_table is not None
    assert isinstance(resolved.data_table.entries[0], ChartDataTableSource)
    assert resolved.data_table.entries[0].format is None
...
def test_resolved_chart_data_table_none_without_authored_data_table():
    from dbt_charts.core.compile.config import get_default_theme_name, get_theme_style
    from dbt_charts.core.compile.models.chart.normalized import BarChart
    from dbt_charts.core.compile.models.query.normalized import SqlQuery
    from dbt_charts.core.compile.resolve import resolve
    ...
    resolved = resolve(chart, [], board_style)
    assert resolved.data_table is None


def test_column_is_numeric_gates_number_default_inheritance():
    # The gate that guards number_default inheritance: only a column whose
...
def test_data_table_entry_format_inherits_style_axis_y_format_at_normalize_time():
