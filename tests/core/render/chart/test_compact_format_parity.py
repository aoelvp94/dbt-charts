...
spec "~s" (trim flag already set, no explicit precision) and produce "1.5M" for
1_500_000. "compact" is an engine-owned predefined name — it is NOT a user alias
in style.formats.
"""
...
def test_compact_vl_axis_format_is_d3_spec():
    ...
    The engine spec for compact is "~s" (trim flag; no explicit precision so Vega's
    d3-scale can supply its own step-derived precision for shared-SI-prefix axes).
    An explicit precision like ".6s" would disable that behavior, mixing magnitudes
    tick-to-tick on a quantitative axis.
    """
    chart = BarChart(
        id="t",
        query=_QUERY,
        ...
        style=BarChartStylePatch.model_validate(
            {"orientation": "vertical", "number_format": "compact"}
        ),
    )
    _rc = resolve(chart, _DATA, chart_style_context=_BOARD_CTX)
    spec = generate_vega_lite_spec(chart, _DATA)
    # "compact" → "~s" (trim, no explicit precision). round_aware_spec is a no-op since ~ is set.
...
def test_compact_kpi_and_axis_share_spec_string_on_round_value():
    ...
    Predefined "compact" resolves to "~s" on both paths; the house notation
    substitution renders "M" → "M" (analytic register for the KPI lane).
    """
    _, number, suffix = format_kpi_parts(1_500_000, "compact")
    assert number + suffix == "1.5M"


def test_compact_format_strings_agree():
    """Predefined 'compact' resolves to the engine spec '~s' on both surfaces."""
    assert resolve_format("compact", None) == "~s"
...
def test_percent_vl_axis_format_lands_on_measure_x_for_horizontal_bar():
    ...
    Before the fix, encoding['x']['axis']['format'] was the theme default '~s' and
