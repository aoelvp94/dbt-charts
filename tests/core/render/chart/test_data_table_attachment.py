"""Tests for the data_table VL emission (render/chart/data_table_attachment.py).
...
def _attach(
    ...
    """Test shim: attach_data_table with a default resolved charts_style."""
    return attach_data_table(
        spec,
        data_table=data_table,
        entry_numerals=plain_numerals(data_table, None) if data_table else (),
        style=style or get_theme_style().charts.data_table,
...
def _attach_per_series(
    ...
    """Shim for per-series attach_data_table calls."""
...
def _axis_label_padding(charts_style):
    ...
    attach_data_table no longer computes this itself (render boundary
...
def _table(entries):
    return ChartDataTable.model_validate({"entries": entries})
...
def test_attach_returns_untouched_when_data_table_is_none():
    spec = _base_spec()
...
def test_attach_is_idempotent_on_empty_entries():
    # ChartDataTable forbids empty lists at validation time, but defensively:
    # caller passing a non-ChartDataTable None-equivalent should still no-op.
    spec = _base_spec()
    out = _attach(spec, None)
...
def test_attach_source_row_emits_text_layer_with_shared_x():
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="revenue", format="$.2s")])
    ...
    assert len(layers) >= 2
    # Find the attached text layer (has a calculate transform on __data_table_*).
    ...
    assert tl["encoding"]["text"]["field"].startswith("__data_table_")
...
def test_attach_source_row_without_format_uses_raw_value():
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="sample_size")])
    out = _attach(spec, table)
    text_layers = _cell_text_layers(out)
    tl = text_layers[0]
...
def test_attach_source_row_without_label_uses_source_display_name():
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="successful_launches")])
    out = _attach(spec, table)
    stub_layers = [
        layer
...
def test_attach_aggregate_row_emits_aggregate_transform_grouped_by_x():
    spec = _base_spec()
    table = _table(
        [ChartDataTableAggregate(aggregate="sum", source="revenue", format="$.2s")]
    )
    out = _attach(spec, table)
    text_layers = _cell_text_layers(out)
    assert len(text_layers) == 1
...
def test_attach_aggregate_row_format_calculate_renders_invalid_values_as_dash():
    ...
    tl = text_layers[0]
    calc = next(t["calculate"] for t in tl.get("transform", []) if "calculate" in t)
    assert "isValid(datum.__data_table_0_val)" in calc
    assert "isFinite(datum.__data_table_0_val)" in calc
    assert "'-'" in calc


def test_attach_aggregate_row_without_label_uses_source_display_name():
    spec = _base_spec()
    table = _table([ChartDataTableAggregate(aggregate="sum", source="total_mass_kg")])
    out = _attach(spec, table)
    stub_layers = [
        layer
        for layer in out["layer"]
...
def test_attach_aggregate_translates_avg_to_mean_at_lowering_layer():
    ...
    spec = _base_spec()
    table = _table([ChartDataTableAggregate(aggregate="avg", source="revenue")])
    out = _attach(spec, table)
    text_layers = _cell_text_layers(out)
...
def test_attach_aggregate_translates_count_distinct_to_distinct():
    spec = _base_spec()
    table = _table(
        [ChartDataTableAggregate(aggregate="count_distinct", source="user_id")]
    )
    out = _attach(spec, table)
    text_layers = _cell_text_layers(out)
    transforms = text_layers[0]["transform"]
...
def test_attach_emits_divider_rule_when_width_positive():
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="revenue")])
...
def test_attach_preserves_base_chart_as_first_layer():
    ...
    table = _table(
        [
            ChartDataTableSource(source="revenue"),
            ChartDataTableAggregate(aggregate="sum", source="revenue"),
        ]
    )
    out = _attach(spec, table)
    first = out["layer"][0]
...
def test_attach_multiple_rows_emits_one_layer_per_row():
    ...
    table = _table(
        [
            ChartDataTableSource(source="a"),
            ChartDataTableSource(source="b"),
            ChartDataTableAggregate(aggregate="sum", source="a"),
        ]
    )
    out = _attach(spec, table)
    text_layers = _cell_text_layers(out)
...
def test_attach_label_stub_layer_defaults_from_source_when_label_is_omitted():
    ...
    table = _table(
        [
            ChartDataTableSource(source="a", label="A"),
            ChartDataTableSource(source="b"),  # label defaults from source
            ChartDataTableSource(source="c", label="C"),
        ]
    )
    out = _attach(spec, table)
...
def test_attach_no_op_when_data_table_is_none_does_not_require_height():
    # The "data_table is None" branch must not enforce height — no strip is
...
def test_attach_requires_explicit_spec_width_when_right_cap_label():
    ...
    table = _table([ChartDataTableSource(source="a", label="A")])
    with pytest.raises(RenderError, match="explicit width") as excinfo:
        attach_data_table(
            spec,
            data_table=table,
            entry_numerals=plain_numerals(table, None),
            style=get_theme_style().charts.data_table,
            charts_style=_charts_style(),
            axis_label_padding=_axis_label_padding(_charts_style()),
...
def test_attach_requires_charts_style_when_data_table_present():
    # charts_style feeds x-axis label offset computation; None with a
    # non-empty data_table has no anchor to compute from.
    from dbt_charts.core.render.errors import RenderError
...
def test_per_series_requires_series_order():
    ...
    table = _table([ChartDataTablePerSeries(per_series="revenue")])
...
def test_per_series_row_layers_requires_explicit_spec_width_when_right_cap_label():
    ...
    spec = _per_series_spec()
    del spec["width"]
    table = _table([ChartDataTablePerSeries(per_series="revenue", label="A")])
    with pytest.raises(RenderError, match="explicit.*width") as excinfo:
        attach_data_table(
            spec,
            data_table=table,
...
def test_attach_omits_row_rule_layers_when_width_zero():
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="a"), ChartDataTableSource(source="b")])
...
def test_attach_label_right_orient_uses_pixel_literal_x():
    ...
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="a", label="A")])
    # Default axis_y_orient="right" (Dataface convention)
    out = _attach(spec, table)
...
def test_default_axis_y_orient_emits_right_cap_no_left_edge_collision():
    ...
    # row.padding.horizontal (inside the plot).
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="a", label="Revenue")])
...
def test_agg_op_map_keys_are_all_authoring_surface_ops():
    ...
    from dbt_charts.core.compile.models.chart.authored import ChartDataTableAggregateOp
    from dbt_charts.core.render.chart.data_table_attachment import _AGG_OP_TO_VL

    authoring_ops = set(get_args(ChartDataTableAggregateOp))
...
def test_attach_does_not_mutate_input_spec_encoding():
    ...
    assert set(spec_in["encoding"].keys()) == encoding_keys_before, (
        f"attach_data_table mutated the input spec.encoding; before="
...
def test_attach_sampling_step_gt1_adds_window_filter_to_text_layers():
    ...
    spec = _temporal_spec()
    table = _table([ChartDataTableSource(source="revenue")])
    step = math.ceil(97 / get_chart_rendering().data_table.chart_data_table_max_x_ticks)
    out = attach_data_table(
        spec,
        data_table=table,
        entry_numerals=plain_numerals(table, None),
    ...
    assert not any(
        "__data_table_row_index" in str(t) for t in base_layer.get("transform", [])
    )
    ...
    for tl in text_layers:
        ...
        assert "__data_table_row_index" in filter_expr
...
def test_attach_sampling_step_1_does_not_add_window_filter():
    ...
    for tl in text_layers:
        ...
        assert not any("window" in t for t in transforms)
        assert not any(
            "filter" in t and "__data_table_row_index" in str(t) for t in transforms
        )
...
def test_attach_aggregate_sampling_places_window_after_aggregate():
    ...
    spec = _temporal_spec()
    table = _table([ChartDataTableAggregate(aggregate="sum", source="revenue")])
    step = math.ceil(80 / get_chart_rendering().data_table.chart_data_table_max_x_ticks)
    out = attach_data_table(
        spec,
        data_table=table,
        entry_numerals=plain_numerals(table, None),
...
def test_per_series_expands_to_n_text_layers():
    ...
    spec = _per_series_spec()
    table = _table([ChartDataTablePerSeries(per_series="revenue", format="$,.0f")])
...
def test_per_series_label_layers_have_colored_fill():
    ...
    attach_data_table no longer resolves bright->dark-companion colours itself
...
def test_per_series_right_orient_label_emits_right_cap():
    ...
    table = ChartDataTable.model_validate({"entries": [{"per_series": "revenue"}]})
    spec = {
        "mark": {"type": "bar"},
        "encoding": {
            "x": {"field": "month", "type": "ordinal"},
...
def test_aggregate_cell_layers_have_color_none_in_encoding():
    ...
    to null — causing endpoint label and data-table dark-companion off-by-one.
    """
    from dbt_charts.core.compile.models.chart.authored import ChartDataTable

    spec = _per_series_layered_spec()
    table = ChartDataTable.model_validate(
        {
            "entries": [
                {
                    "aggregate": "sum",
...
def test_right_orient_label_anchors_at_spec_width_plus_axis_padding():
    ...
    spec = _base_spec()
    table = _table([ChartDataTableSource(source="revenue", label="Revenue")])
    out = attach_data_table(
        spec,
        data_table=table,
        entry_numerals=plain_numerals(table, None),
...
def test_right_orient_label_sets_mark_limit():
    ...
    from dbt_charts.core.render.chart.data_table_attachment import _LABEL_STUB_LIMIT_PX
...
def test_value_cells_align_toward_label_column():
    ...
    table = _table(
        [ChartDataTableAggregate(source="revenue", aggregate="sum", format="$,.0f")]
    )
...
def test_by_measure_label_suppressed_when_redundant():
    ...
    table = _table([ChartDataTablePerSeries(per_series="revenue", by_measure=True)])
    out = _attach_per_series(
        _per_series_spec(),
        table,
...
def test_aggregate_label_not_suppressed_by_series_label_flag():
    ...
    table = _table(
        [ChartDataTableAggregate(source="revenue", aggregate="sum", label="Total")]
    )
    out = _attach(_base_spec(), table, suppress_series_labels=True)
    assert len(_label_layers(out)) == 1


def test_data_table_strip_height_returns_zero_for_no_table():
    assert data_table_strip_height(None, _dt_style(), _charts_style()) == 0.0


def test_data_table_strip_height_scales_with_row_count():
    style = _dt_style()
    cs = _charts_style()
    one = data_table_strip_height(_table([ChartDataTableSource(source="a")]), style, cs)
    three = data_table_strip_height(
        _table(
            [
                ChartDataTableSource(source="a"),
                ChartDataTableSource(source="b"),
...
def test_data_table_strip_height_includes_axis_offset_for_bottom():
    ...
    from dbt_charts.core.render.chart.data_table_attachment import _strip_height

    style = _dt_style(position="bottom")
    cs = _charts_style()
    table = _table([ChartDataTableSource(source="a")])
    h = data_table_strip_height(
...
def test_per_series_height_budget_counts_n_rows():
    """data_table_strip_height must count per_series as N rows, not 1."""
    style = _dt_style()
    cs = _charts_style()

    # 3 normal source rows.
    three_source = data_table_strip_height(
        _table(
            [
                ChartDataTableSource(source="a"),
                ChartDataTableSource(source="b"),
    ...
    one_per_series = data_table_strip_height(
        _table([ChartDataTablePerSeries(per_series="revenue")]),
        style,
        cs,
        series_count=3,
    )
...
def test_per_series_height_raises_without_series_count():
    ...
    with pytest.raises(RenderError, match="series_count"):
        data_table_strip_height(table, style, cs)
    with pytest.raises(RenderError, match="series_count"):
        data_table_strip_height(table, style, cs, series_count=0)


def test_bottom_position_strip_height_drives_padding_bottom():
    # data_table_strip_height for position:bottom returns axis_offset + strip_height.
    from dbt_charts.core.compile.data_table import axis_offset
    from dbt_charts.core.render.chart.data_table_attachment import _strip_height
...
def test_top_position_strip_height_excludes_axis_offset():
    ...
    # above the strip. data_table_strip_height must return _strip_height only.
    from dbt_charts.core.compile.data_table import axis_offset
    from dbt_charts.core.render.chart.data_table_attachment import _strip_height
    ...
    table = _table([ChartDataTableSource(source="a")])
    h = data_table_strip_height(table, style, None)
    ...
    bottom_h = data_table_strip_height(
        table,
        bottom_style,
...
def test_strip_height_and_row_y_use_label_max_lines():
    # data_table_strip_height and _row_y_pixel must both honour label_max_lines
    ...
    from dbt_charts.core.compile.data_table import axis_offset
    from dbt_charts.core.render.chart.data_table_attachment import _row_y_pixel
...
def test_validate_temporal_over_40_returns_step_and_does_not_raise():
    ...
    table = _table([ChartDataTableSource(source="revenue")])
    step = validate_data_table_against_data(table, "date", data, x_type="temporal")
    assert step == math.ceil(
        97 / get_chart_rendering().data_table.chart_data_table_max_x_ticks
    )
    assert step > 1
...
def test_validate_quantitative_over_40_returns_step_and_does_not_raise():
    ...
    table = _table([ChartDataTableSource(source="revenue")])
    step = validate_data_table_against_data(table, "x_val", data, x_type="quantitative")
    assert step == math.ceil(
        80 / get_chart_rendering().data_table.chart_data_table_max_x_ticks
    )
    assert step > 1
...
def test_validate_ordinal_over_40_still_raises():
    ...
    with pytest.raises(RenderError, match="40") as excinfo:
        validate_data_table_against_data(table, "month", data, x_type="ordinal")
    assert excinfo.value.code is not None
    assert excinfo.value.code.code == "ERR-INPUT-INVALID"
...
def test_validate_no_x_type_over_40_still_raises():
    ...
    table = _table([ChartDataTableSource(source="revenue")])
    with pytest.raises(RenderError, match="40") as excinfo:
        validate_data_table_against_data(table, "month", data)
    assert excinfo.value.code is not None
    assert excinfo.value.code.code == "ERR-INPUT-INVALID"
...
def test_validate_temporal_over_40_still_checks_source_column_existence():
    ...
    table = _table([ChartDataTableSource(source="revenu")])
    with pytest.raises(RenderError, match="revenu") as excinfo:
        validate_data_table_against_data(table, "date", data, x_type="temporal")
    assert excinfo.value.code is not None
    assert excinfo.value.code.code == "ERR-INPUT-INVALID"
...
def test_validate_temporal_over_40_still_checks_multi_row_ambiguity():
    ...
    table = _table([ChartDataTableSource(source="revenue")])  # bare source, not agg
...
# =============================================================================
# Width-aware thinning + format inheritance (DFT_CORE-DATA_TABLE_STRIP_OVERLAPS)
# =============================================================================
...
def test_wide_currency_cells_dense_lex_ordinal_axis_triggers_width_thinning():
    ...
    chart = TypeAdapter(Chart).validate_python(
        {
            ...
            "data_table": [{"source": "revenue", "format": "$,.2f", "label": "Rev"}],
    ...
    assert text_layers, "data_table strip must produce text layers"
    for layer in text_layers:
        transforms = layer.get("transform", [])
    ...
    chart = TypeAdapter(Chart).validate_python(
        {
            ...
            "query": SqlQuery(sql="SELECT 1", source="test_db"),
            "query_name": "q",
            "data_table": [{"source": "revenue", "format": "$,.2f"}],
            "style": {"orientation": "vertical"},
        }
    )
...
def test_strip_cell_no_format_on_y_measure_inherits_number_default():
    # Strip cell with no authored format and source == y field must inherit
    # the theme's compact number_default rather than rendering raw/full-precision.
    # When format is None, _vl_format_calc emits `datum.field ? datum.field : '-'`
    # (no format() call). After the fix, the entry inherits number_default and
    ...
    # No explicit format on the data_table entry, no authored style.number_format.
    chart = TypeAdapter(Chart).validate_python(
        {
            "id": "no_format_bar",
            "type": "bar",
            ...
            "query": SqlQuery(sql="SELECT 1", source="test_db"),
            "query_name": "q",
            "data_table": [{"source": "revenue"}],  # format intentionally omitted
            "style": {"orientation": "vertical"},
        }
    )
    ...
    assert calc_transforms, "data_table strip must emit calculate transforms"
    ...
    assert has_format_call, (
        ...
        "number_default so the calculate transform uses format(...). "
...
def test_strip_cell_no_format_on_string_y_not_grafted_with_numeric_format():
    ...
    # y is a string column (dot plot); the data_table source == y is non-numeric.
    chart = TypeAdapter(Chart).validate_python(
        {
            "id": "string_y_scatter",
            "type": "scatter",
            ...
            "query": SqlQuery(sql="SELECT 1", source="test_db"),
            "query_name": "q",
            "data_table": [{"source": "grade"}],  # format omitted, non-numeric
        }
    )
    spec = generate_vega_lite_spec(chart, data, width=400, height=300)
