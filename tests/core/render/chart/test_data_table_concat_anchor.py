"""The data_table strip stays anchored to the plot it hangs off.

``attach_data_table`` positions every strip layer with a pixel-literal y
    ...
    return theme.model_copy(
        update={
            "charts": charts.model_copy(
                update={
                    "data_table": charts.data_table.model_copy(
...
def _render(make_chart, data, with_color: bool, with_strip: bool):
    ...
    if with_color:
        kwargs["color"] = "series"
    if with_strip:
        kwargs["data_table"] = ChartDataTable(
            entries=[ChartDataTableAggregate(aggregate="sum", source="value")]
        )
    resolved = resolve(
...
def test_concat_chart_without_a_strip_keeps_target_height(make_chart):
    ...
    Endpoint-label charts with no data_table have no pixel-literal anchors, so
