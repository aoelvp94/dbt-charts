"""Render-path tests: per_series data_table labels and legend interaction.

When a per_series data_table is attached and the chart has a visible color
...
With endpoint labels both label pane and data table labels are visible.
...
def _chart_spec(
    ...
    if data_table is None:
        data_table = {"entries": [{"per_series": "rev"}]}
    payload: dict[str, Any] = {
        ...
        "query_name": "q",
        "data_table": ChartDataTable.model_validate(data_table),
    }
    if chart_type == "bar":
...
def test_per_series_labels_shown_legend_suppressed():
    # Legend visible + per_series data_table → labels SHOW, legend suppressed.
    # The data table provides the series legend; a side legend is redundant.
    spec = _chart_spec()
    assert _label_layers(spec), "per_series labels must be visible in the strip"
    assert _color_legend(spec) is None, (
        "color legend must be suppressed when per_series data_table is present; "
        f"got legend={_color_legend(spec)!r}"
    )


def test_aggregate_data_table_does_not_suppress_legend():
    ...
    assert color_enc.get("legend", "visible") is not None, (
        "aggregate-only data_table must not suppress the color legend; "
...
def test_bar_stacked_value_order_smallest_sum_first():
    ...
    # Data table row order must be top-to-bottom, so smallest sum = row 0.
    ...
    chart = BarChart(
        ...
        query=SqlQuery(sql="SELECT 1", source="src"),
        query_name="q",
        data_table=ChartDataTable.model_validate({"entries": [{"per_series": "rev"}]}),
        style={"orientation": "vertical"},
    )
    spec = generate_vega_lite_spec(
        chart,
